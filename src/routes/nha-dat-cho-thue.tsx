...
export const Route = createFileRoute("/nha-dat-cho-thue")({
  validateSearch: (search: Record<string, unknown>): NhaDatChoThueSearch => ({
  ...
  component: NhaDatChoThuePage,
  head: () => ({
    meta: [
      { title: "Nhà đất cho thuê Đà Nẵng — Cho thuê căn hộ, mặt bằng, nhà riêng" },
      {
        name: "description",
        content:
          "Danh sách cho thuê nhà riêng, mặt bằng kinh doanh, căn hộ chung cư cao cấp tại Đà Nẵng giá tốt nhất.",
      },
    ],
  }),
});

function NhaDatChoThuePage() {
  ...
  // Execute client-side filtering via custom hook for rental properties
  const rawFiltered = useFilteredListings(sampleListings, currentFilters, "rent");
  ...
  // Filter change handlers updating URL params
  ...
  const handleContactSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      toast.success("Yêu cầu thuê nhà đã được gửi! Môi giới sẽ gọi lại ngay.");
        ...
        <div className="bg-surface-container/50 border-b border-border/60 py-6 md:py-8">
          <div className="container-page">
            <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
              ...
              <span>/</span>
              <span className="text-foreground font-semibold">Nhà đất cho thuê</span>
            </div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Tìm thấy <span className="font-bold text-primary">{filteredListings.length}</span> bất động sản cho thuê phù hợp.
            </p>
          </div>
        </div>
            ...
            <div className="lg:col-span-3 space-y-4">
              <div className="flex items-center justify-between gap-4 bg-card border border-border/70 rounded-xl p-3 px-4 shadow-xs">
                <div className="text-sm font-semibold text-foreground">
                  Tìm thấy <span className="text-primary font-bold">{filteredListings.length}</span> tin cho thuê
                </div>
              ...
              {filteredListings.length === 0 ? (
                <div className="bg-card border border-border/80 rounded-2xl p-12 text-center my-6 shadow-xs flex flex-col items-center justify-center">
                  ...
                  <h3 className="text-base font-bold text-foreground mb-1">
                    Không tìm thấy bất động sản cho thuê phù hợp với bộ lọc
                  </h3>
                  <p className="text-xs text-muted-foreground max-w-md mb-6 leading-relaxed">
                    Hãy thử thay đổi điều kiện hoặc bấm nút xóa bộ lọc để hiển thị toàn bộ danh sách.
                  </p>
                  <Button
        ...
        <DialogContent className="max-w-4xl p-0 overflow-hidden border border-border/80 bg-card rounded-2xl shadow-2xl max-h-[90vh] flex flex-col">
          <DialogHeader className="p-6 pb-0 border-b border-border/50 shrink-0">
            <div className="flex items-center gap-2">
              ...
              {selectedProperty?.isHot && (
                <span className="bg-rose-500 text-white font-bold text-[10px] px-2 py-0.5 rounded">
                  HOT
                </span>
              )}
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div className="bg-surface-container/50 border border-border/40 rounded-xl p-3 text-center">
                  <div className="text-xs text-muted-foreground">Giá cho thuê</div>
            ...
            <div className="flex flex-col space-y-6">
              <div>
                <h4 className="font-bold text-sm text-foreground uppercase tracking-wider mb-2">Mô tả bất động sản</h4>
              ...
              <div className="border border-border/50 bg-surface-container/20 rounded-xl p-5">
                ...
                <form onSubmit={handleContactSubmit} className="space-y-3">
                  ...
                  >
                    {isSubmittingContact ? "Đang gửi..." : "Liên hệ xem nhà ngay"}
                  </Button>
                </form>
              </div>
            </div>
